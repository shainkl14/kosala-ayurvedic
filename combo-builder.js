document.addEventListener('DOMContentLoaded', () => {
    ...
    const phoneNumber = '919526616666'; // Your WhatsApp number
    ...
    function getDiscount(totalQuantity) {
        if (totalQuantity >= 5) return 0.15; // 15% off for 5 or more items
        if (totalQuantity >= 3) return 0.10; // 10% off for 3-4 items
        if (totalQuantity >= 2) return 0.05; // 5% off for 2 items
        return 0; // 0% discount for 1 item
    }
    ...
    function updateSummary() {
        ...
        discountAmountEl.textContent = `- ₹${discountAmount} (${discountRate * 100}%)`;
    ...
    function generateWhatsAppLink() {
        ...
        message += `\nTotal: ₹${finalPriceEl.textContent.replace('₹', '')}`;
        const encodedMessage = encodeURIComponent(message);
        return `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
    }
});
// --- LOGIC FOR COMBO SLIDER ---
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!sliderWrapper) return; // Only run if the slider is on the page
